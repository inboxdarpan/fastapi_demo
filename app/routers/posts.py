...
from typing import Optional, List #Optional is for optional fields
...
def get_posts(new_post: schemas.PostCreate, db: Session = Depends(get_db), user: int = Depends(oauth2.get_current_user)): #payload: dict = Body(...)
    ...
    created_post = models.Post(owner_id=user.id, **new_post.model_dump())
    db.add(created_post)
    db.commit()
...
def get_one_post(id: int, db: Session = Depends(get_db), user_id: int = Depends(oauth2.get_current_user)): #response: Response was there where we tried editing the status code in the response
    ...
    # post = db.fetchone()
    post = db.query(models.Post).filter(schemas.Post.id==id, models.Post.owner_id).first() #note: used db.query()
...
def get_all_posts(db: Session = Depends(get_db), user: int = Depends(oauth2.get_current_user), limit: int= 3):
    # posts = db.query(models.Post).limit(limit).all()
    posts = db.query(models.Post).filter(models.Post.owner_id==user.id).limit(limit=limit).all()
...
def get_post(id: int, new_post: schemas.Post, db: Session=Depends(get_db), user_id: int = Depends(oauth2.get_current_user)):
